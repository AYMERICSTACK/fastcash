...
export function offerReceivedCustomerEmail({productName,offeredPrice,name}:{productName:string;offeredPrice:number;name:string}){
 return baseEmailLayout({preheader:"Votre offre a bien été reçue.",eyebrow:"Offre reçue",title:"Votre proposition est entre de bonnes mains",intro:`Bonjour ${name}, votre offre de ${formatMoney(offeredPrice,"CHF")} pour ${productName} a bien été transmise à l'équipe FAST CASH Genève.`,content:`<div style="padding:18px;border-radius:16px;background:#fbfaf7;border:1px solid #eee;color:#555;font-size:14px;line-height:1.75;">Nous allons l'étudier. Vous recevrez un nouvel e-mail si elle est acceptée ou si une contre-offre vous est proposée.</div>`});
}
export function offerReceivedAdminEmail({offerId,productName,listPrice,offeredPrice,name,email,phone,message}:{offerId:string;productName:string;listPrice:number;offeredPrice:number;name:string;email:string;phone:string|null;message:string|null}){
 return baseEmailLayout({preheader:`Nouvelle offre de ${name}.`,eyebrow:"Nouvelle négociation",title:"Une nouvelle offre client est arrivée",intro:`${name} propose ${formatMoney(offeredPrice,"CHF")} pour ${productName}.`,content:`${infoCard("Prix affiché",formatMoney(listPrice,"CHF"))}<div style="height:10px"></div>${infoCard("Offre client",formatMoney(offeredPrice,"CHF"),true)}<div style="margin-top:18px;color:#555;line-height:1.7"><b>Client :</b> ${escapeHtml(name)}<br/><b>E-mail :</b> ${escapeHtml(email)}${phone?`<br/><b>Téléphone :</b> ${escapeHtml(phone)}`:""}${message?`<br/><b>Message :</b> ${nl2br(message)}`:""}</div>`,cta:{label:"Traiter l'offre",href:`${getSiteUrl()}/pilotage/offres/${offerId}`}});
}
export function negotiatedOfferEmail({name,productName,price,link,isCounter}:{name:string;productName:string;price:number;link:string;isCounter:boolean}){
 return baseEmailLayout({preheader:isCounter?"FAST CASH vous propose un nouveau prix.":"Votre offre a été acceptée.",eyebrow:isCounter?"Contre-offre privée":"Offre acceptée",title:isCounter?"Une proposition FAST CASH vous attend":"Votre offre est acceptée",intro:`Bonjour ${name}, ${isCounter?"FAST CASH vous propose":"votre offre est validée au prix de"} ${formatMoney(price,"CHF")} pour ${productName}.`,content:`<div style="padding:18px;border-radius:16px;background:#fff8df;border:1px solid #d4af37;color:#444;font-size:14px;line-height:1.75;">Ce lien est personnel, sécurisé et limité dans le temps. Le prix public du produit reste inchangé : votre tarif négocié est appliqué uniquement via ce lien.</div>`,cta:{label:`Acheter à ${formatMoney(price,"CHF")}`,href:link}});
}
export function offerRefusedEmail({name,productName}:{name:string;productName:string}){return baseEmailLayout({preheader:"Mise à jour de votre offre FAST CASH.",eyebrow:"Votre offre",title:"Merci pour votre proposition",intro:`Bonjour ${name}, après étude, nous ne pouvons pas accepter votre proposition pour ${productName}.`,content:`<div style="padding:18px;border-radius:16px;background:#fbfaf7;border:1px solid #eee;color:#555;font-size:14px;line-height:1.75;">Le produit reste disponible au tarif affiché sur le site, sous réserve du stock boutique.</div>`});}
